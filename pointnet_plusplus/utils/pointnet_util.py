...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if pooling == 'avg':
            ...
            new_points = tf_util.max_pool2d(-1 * new_points, [1, nsample], stride=[1, 1], padding='VALID',
                                            scope='minpool1')
        elif pooling == 'max_and_avg':
...
def pointnet_deconv(inputs, num_output_channels, n_upsample, is_training, bn_decay, scope):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        new_points = tf_util.conv2d_transpose(inputs, num_output_channels, [1, n_upsample], scope='deconv',
                                              stride=[1, n_upsample], bn_decay=bn_decay)
