...
def create_model( FLAGS ):
    ...
    learning_rate_G = tf.maximum(learning_rate_G, 1e-4)
    ...
    ################### define my losses #############################
    gen_geo_loss, gen_shapeLoss, gen_densityLoss = get_Geometric_Loss(predictedSet, pointSet_out_ph, FLAGS)
    transform_Loss = get_transform_Loss(predictedSet, pointSet_in_ph, FLAGS)

    gen_loss = gen_geo_loss + transform_Loss*2
