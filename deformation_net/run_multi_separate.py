...
parser.add_argument('--train_hdf5', default='../data_hdf5/multi_train.hdf5')
parser.add_argument('--test_hdf5', default='../data_hdf5/multi_test.hdf5')
...
print (Test_examples.names)
## 取出来马
idx_horse_train = [i for i in range(Train_examples.names.shape[0]) if 'mesh_00' in Train_examples.names[i]  ]

Examples = collections.namedtuple("Examples", "names, skeleton_in, pointSet_out")
skeleton_in_h = Train_examples.skeleton_in[idx_horse_train,:]
pointSet_out_h = Train_examples.pointSet_out[idx_horse_train,:]
names_h = Train_examples.names[idx_horse_train]
Horse_Train_examples = Examples(
        names = names_h,  skeleton_in = skeleton_in_h,
        pointSet_out=pointSet_out_h,
    )

## 取出来狮子
idx_lioness_train = [i for i in range(Train_examples.names.shape[0]) if 'mesh_10' in Train_examples.names[i]  ]

skeleton_in_l = Train_examples.skeleton_in[idx_lioness_train,:]
pointSet_out_l = Train_examples.pointSet_out[idx_lioness_train,:]
names_l = Train_examples.names[idx_lioness_train]
Lioness_Train_examples = Examples(
        names = names_l,  skeleton_in = skeleton_in_l,
        pointSet_out=pointSet_out_l,
    )

## 取出来狗
idx_dog_train = [i for i in range(Train_examples.names.shape[0]) if 'mesh_20' in Train_examples.names[i]  ]

skeleton_in_d = Train_examples.skeleton_in[idx_dog_train,:]
pointSet_out_d = Train_examples.pointSet_out[idx_dog_train,:]
names_d = Train_examples.names[idx_dog_train]
Dog_Train_examples = Examples(
        names = names_d,  skeleton_in = skeleton_in_d,
        pointSet_out=pointSet_out_d,
    )

'''重新排列数据'''
Horse_Train_examples = ioUtil.arrange_datas(Horse_Train_examples)
Lioness_Train_examples = ioUtil.arrange_datas(Lioness_Train_examples)
Dog_Train_examples = ioUtil.arrange_datas(Dog_Train_examples)

skeleton_in_m = np.append(Lioness_Train_examples.skeleton_in, Horse_Train_examples.skeleton_in, axis=0)
pointSet_out_m = np.append(Lioness_Train_examples.pointSet_out, Horse_Train_examples.pointSet_out, axis=0)
pointSet_in_m = np.append(Lioness_Train_examples.pointSet_in, Horse_Train_examples.pointSet_in, axis=0)
names_m = np.append(Lioness_Train_examples.names, Horse_Train_examples.names, axis=0)

skeleton_in_m = np.append(skeleton_in_m, Dog_Train_examples.skeleton_in, axis=0)
pointSet_out_m = np.append(pointSet_out_m, Dog_Train_examples.pointSet_out, axis=0)
pointSet_in_m = np.append(pointSet_in_m, Dog_Train_examples.pointSet_in, axis=0)
names_m = np.append(names_m, Dog_Train_examples.names, axis=0)

Train_data = ioUtil.Datas(
        names = names_m,  skeleton_in = skeleton_in_m,
        pointSet_out=pointSet_out_m, pointSet_in=pointSet_in_m
    )
...
##################### output data #######################
output_dir = 'output_multi_seperate'
